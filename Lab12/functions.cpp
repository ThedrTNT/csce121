...
int countNeighborMines(struct minePosition** mineMatrix, int nRows, int nCols, int row, int col)
{
  ...
  if (row < nRows && row >= 0 && col < nCols && col >= 0) {
    if ((row - 1 >= 0 && col - 1 >= 0) && mineMatrix[row - 1][col - 1].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
    if ((row >= 0 && col - 1 >= 0) && mineMatrix[row ][col - 1].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
    if ((row + 1 < nRows && col - 1 >= 0) && mineMatrix[row + 1][col - 1].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
    if ((row - 1 >= 0 && col >= 0) && mineMatrix[row - 1][col].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
    if ((row + 1 < nRows && col >= 0) && mineMatrix[row + 1][col].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
    if ((row - 1 >= 0 && col + 1 < nCols) && mineMatrix[row - 1][col + 1].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
    if ((row >= 0 && col + 1 < nCols) && mineMatrix[row ][col + 1].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
    if ((row + 1 < nRows && col + 1 < nCols) && mineMatrix[row + 1][col + 1].neighborMines == MINE) {
      numberOfNeighborMines++;
    }
  }
...
int openPosition(struct minePosition** mineMatrix, int nRows, int nCols, int row, int col)
{
  ...
  if (row >= 0 && row < nRows && col >= 0 && col < nCols) {
    ...
    else if (countNeighborMines(mineMatrix, nRows, nCols, row, col) != 0) {
      mineMatrix[row][col].open = true;
      mineMatrix[row][col].neighborMines = countNeighborMines(mineMatrix, nRows, nCols, row, col);
    }
    else {
      //cout << "opening neighbors" << endl;
      mineMatrix[row][col].open = true;
      for (int i = row - 1; i <= row + 1; ++i) {
        for (int j = col - 1; j <= col + 1; ++j) {
          if (i >= 0 && i < nRows && j >= 0 && j < nCols) {
            if (mineMatrix[i][j].open != true) {
