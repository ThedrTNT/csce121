...
Image::Image(const Image& source) : image(nullptr), width(source.width), height(source.height){
  allocateImage(source.width, source.height);
  for (unsigned int i = 0; i < source.width; ++i) {
    for (unsigned int j = 0; j < source.height; ++j) {
      image[i][j] = source.image[i][j];
    }
  }
}
